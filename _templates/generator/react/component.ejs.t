...
class <%= Name %> extends Component {
  ...
  render = () => {
    return (
      <div><%= h.inflection.singularize(h.capitalize(section)) %> at app/<%= section %>/<%= name %>/component.js</div>
    )
  }
}
<% } else if (locals.type === 'functional') { -%>

const <%= Name %> = props => <div><%= h.inflection.singularize(h.capitalize(section)) %> at app/<%= section %>/<%= name %>/component.js</div>
<% } else { -%>
